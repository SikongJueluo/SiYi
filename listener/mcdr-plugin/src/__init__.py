...
def on_info(server: PluginServerInterface, info: Info):
    ...
    if not info.is_user and re.fullmatch(
        r"Starting Minecraft server on \S*",
        info.content if info.content is not None else "",
    ):
        server.logger.info(
            "Minecraft is starting at address {}".format(info.content.rsplit(" ", 1)[1])
        )
...
def on_server_stop(server: PluginServerInterface, return_code: int):
    ...
    """
    server.logger.info(
        "Server has stopped and its return code is {}".format(return_code)
    )
